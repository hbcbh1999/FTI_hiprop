...
static int32_T b_obtain_nring_surf(real_T vid, int32_T ring, int32_T minpnts,
  ...
  const emxArray_boolean_T *ftags)
{
  ...
  boolean_T b1;
  int32_T fid_in;
  static const int8_T iv4[3] = { 2, 3, 1 };

  int32_T hebuf[128];
  int32_T exitg4;
  static const int8_T iv5[3] = { 3, 1, 2 };
  ...
  fid = (int32_T)((uint32_T)v2he->data[(int32_T)vid - 1] >> 2U) - 1;
  ...
  lid = (int32_T)((uint32_T)v2he->data[(int32_T)vid - 1] & 3U);
  ...
  if (!(fid + 1 != 0)) {
    ...
    if (opphes->data[fid + opphes->size[0] * lid] != 0) {
      ...
      ngbvs[0] = tris->data[fid + tris->size[0] * (iv4[lid] - 1)];
    ...
    do {
      ...
      if ((nverts < 128) && (nfaces < 256)) {
        ...
        if (!b1) {
          ...
          hebuf[nverts - 1] = opphes->data[fid + opphes->size[0] * (iv5[lid_prv]
            - 1)];
    ...
    if (ring == 1) {
      ...
      b_vtags->data[(int32_T)vid - 1] = TRUE;
      ...
      do {
          ...
          while (nfaces_pre + 1 <= fid_in) {
            ...
            while ((exitg2 == 0U) && (lid + 1 < 4)) {
              ...
              if ((opphes->data[(ngbfs[nfaces_pre] + opphes->size[0] * lid) - 1]
                ...
                if ((!b_vtags->data[tris->data[fid + tris->size[0] * (iv5[lid] -
                      1)] - 1]) && (!overflow)) {
                  /* 'obtain_nring_surf:172' nverts = nverts + 1; */
                  nverts++;

                  /* 'obtain_nring_surf:172' ngbvs( nverts) = v; */
                  ngbvs[nverts - 1] = tris->data[fid + tris->size[0] * (iv5[lid]
                    - 1)];

                  /* 'obtain_nring_surf:173' vtags(v) = true; */
                  b_vtags->data[tris->data[fid + tris->size[0] * (iv5[lid] - 1)]
                    - 1] = TRUE;
                }
        ...
        if (guard1 == TRUE) {
          ...
          while (nverts_pre + 1 <= nverts_last) {
            ...
            if (opphes->data[fid + opphes->size[0] * lid] != 0) {
              ...
              if (!overflow) {
                ...
                ngbvs[nverts - 1] = tris->data[fid + tris->size[0] * (iv4[lid] -
                  1)];

                /* 'obtain_nring_surf:213' vtags(v)=true; */
                b_vtags->data[tris->data[fid + tris->size[0] * (iv4[lid] - 1)] -
                  1] = TRUE;
          ...
          if (((nverts >= minpnts) && (cur_ring >= (real_T)ring)) || (nfaces ==
               nfaces_pre) || overflow) {
            exitg1 = 1U;
          } else {
  ...
  emxArray_real_T *ring_sz)
{
  ...
  while (nx <= nv_clean) {
    ...
    nverts = c_obtain_nring_surf(nx, ring_sz->data[nx - 1], 0, elems, opphes, v2he,
      unusedU0, r0, r1);
  ...
  while (ii <= (uint32_T)part_bdry->size[0]) {
    ...
    if ((part_bdry->data[(int32_T)ii - 1] != 0) && (ring_sz->data[(int32_T)ii -
         1] == 2.0)) {
      /* 'obtain_ringsz_cleanmesh:58' [ngbvs, nverts] = obtain_nring_surf(ii, int32(1), minpnts, ... */
      ...
      nverts = b_obtain_nring_surf((real_T)ii, 1, 0, elems, opphes, v2he, ngbvs,
        r2, r3);
